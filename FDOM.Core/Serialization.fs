...
module JsonHelpers =
    ...
    let tryGetElementsProperty (name : string) (element : JsonElement) =
        match tryGetProperty name element with
        | Some p -> Some (element.EnumerateObject() |> List.ofSeq)
        | None -> None
    
    let tryGetArrayProperty (name : string) (element : JsonElement) =    
        match tryGetProperty name element with
        | Some p -> Some (element.EnumerateArray() |> List.ofSeq)
        | None -> None
...
module private Internal =
    ...
    module ErrorMessages =
        ...
        let missingProperty name propertyName = $"`{name}` style types require a `{propertyName}` property."
    
        let missingTypeSpecificProperty typeName name propertyName = $"`{name}` { typeName } types require a `{propertyName}` property."
    ...
    type DOM.Style with
        ...
        static member FromJson(element : JsonElement) =
            ...
            | Some t ->
                ...
                | "custom" ->
                    ...
                    | Some p -> 
                        Ok (DOM.Style.Custom (propertiesToStringMap p))
        ...
        member style.WriteToJson(writer : Utf8JsonWriter) =
            ...
            | DOM.Style.Custom map ->
                ...
                map |> Map.map(fun k v -> writer.WriteString(k, v)) |> ignore
                writer.WriteEndArray()
            | DOM.Style.Ref classes ->
                writer.WriteString("type", "ref")
                writer.WriteStartArray("selectors")
                classes |> List.map writer.WriteStringValue |> ignore
                writer.WriteEndArray()
            | DOM.Style.Default ->
    ...
    type DOM.InlineSpan with
         ...
         static member FromJson(element : JsonElement) =
            ...
            | Some s, Some se -> Ok ({ Content = s; Style = DOM.Style.FromJsonWithDefault se } : DOM.InlineSpan)                
            | Some s, None -> Ok ({ Content = s; Style = DOM.Style.Default } : DOM.InlineSpan)
    ...
    type DOM.HeaderBlock with
        ...
        static member FromJson(element : JsonElement) =
            ...
            let indexed = tryGetBoolProperty "indexed" element |> Option.defaultValue false
            ...
            | Some hl, Some se, Some ce ->
                ...
                let (content, errors) = ce |> List.map DOM.InlineContent.FromJson |> collectResults
        ...
        member header.WriteToJson(writer : Utf8JsonWriter) =
            ...
            writePropertyObject header.WriteToJson "style" writer
            writeArray (fun w -> header.Content |> List.map (fun c -> writeObject c.WriteToJson w) |> ignore) "content" writer
    ...
    type DOM.ParagraphBlock with
        ...
        member paragraph.WriteToJson(writer : Utf8JsonWriter) =
            ...
            writePropertyObject paragraph.WriteToJson "style" writer
            writeArray (fun w -> paragraph.Content |> List.map (fun c -> writeObject c.WriteToJson w) |> ignore) "content" writer
    ...
    type DOM.ListItem with
        ...
        member item.WriteToJson(writer : Utf8JsonWriter) =
            writePropertyObject item.WriteToJson "style" writer
            writeArray (fun w -> item.Content |> List.map (fun c -> writeObject c.WriteToJson w) |> ignore) "content" writer
    ...
    type DOM.ListBlock with
        ...
        static member FromJson(element : JsonElement) =
            ...
            | Some o, Some i, Some se ->
                let (items, errors) = i |> List.map DOM.ListItem.FromJson |> collectResults
                Ok({
                    Style = DOM.Style.FromJsonWithDefault se
                    Ordered = o
                    Items = items
        ...
        member list.WriteToJson(writer : Utf8JsonWriter) =
            ...
            writePropertyObject list.WriteToJson "style" writer
            writeArray (fun w -> list.Items |> List.map (fun c -> writeObject c.WriteToJson w) |> ignore) "items" writer
...
type Serializer() =
    ...
    static member Serialize(document : DOM.Document) =
        ""
